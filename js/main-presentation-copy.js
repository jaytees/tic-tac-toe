$(document).ready(function() {
  ...
  $(".squares").on("click", function(ev) {
    ...
    if (ifTaken === false && winner === false) {
      //checks square available to play and no winner
      ...
      $("#message")
        .text("PLEASE RESET BOARD TO PLAY AGAIN! ")
        .css({
          color: "#ff5454",
          visibility: "visible",
          "font-size": "17px",
  ...
  const squareChecker = function(squareClicked) {
    ...
    if (contents === "O" || contents === "X") {
      $("#message")
        .text("PLEASE SELECT ANOTHER SQUARE")
        .css({
          color: "#ff5454",
          visibility: "visible",
          "font-size": "17px",
  ...
  const playerIdentifier = function(player) {
    if (player === 1) {
      //statements are opposite to turn logic so it alternates after every squareClicked
